...
test("should book hotel", async ({ page }) => {
  ...
  await expect(page.getByText("Total Cost: $180.00")).toBeVisible();
  ...
  await stripeFrame.locator('[placeholder="CVC"]').fill("155");
  await stripeFrame.locator('[placeholder="ZIP"]').fill("45754");
...
test("should show past bookings", async ({ page }) => {
  await page.goto(UI_URL);
